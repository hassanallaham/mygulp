...
const DEFAULT_CONFIG = {
	...
	COMPATIBILITY: ['last 2 versions', 'ie >= 9', 'ios >= 7', 'android >= 4.4'],
	UNCSS: {
		enabled: false,
		html: `dist/**/*.html`,
		ignore: [/^.is-.*/gi],
...
let pluginConfigs = util.getPlugins().map((p) => {
	...
	return {
		PATHS: {
			...
			entries: _.get(cfg, 'PATHS.entries', []).map(
				(s) => `node_modules/${p}/src/assets${s}`
			),
			styles: _.get(cfg, 'PATHS.styles', []).map(
				(s) => `node_modules/${p}/src/assets${s}`
			),
		},
		CDN: {
...
if (!CUSTOM_CONFIG.ORIGIN && CUSTOM_CONFIG.PORT)
