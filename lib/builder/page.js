...
class Page {
	constructor(site, file) {
		...
		if (this.data.url) {
			if (this.data.url === '/') {
				this.path = path.join(process.cwd(), site.root, 'index.html');
			} else {
				this.path = path.join(
					process.cwd(),
					site.root,
				...
				if (this.data.url.substr(-1) != '/')
					this.data.url = this.data.url + '/';
			}
		} else {
			this.data.url = path
				.join(site.root, path.basename(file.path))
	...
	getPathFromUrl(pageUrl) {
		...
		if (pageUrl === '/') {
			pagePath = path.join(process.cwd(), this.site.root, 'index.html');
		} else {
			pagePath = path.join(
				process.cwd(),
				this.site.root,
	...
	static fromHelper(context, pageName) {
		...
		let pPath = util.path(
			path.join(
				...
				'/' + (pageName === 'index' ? '' : pBase.join(path.sep))
			)
		);
		let found = util.loadFiles(pPath, `${pName}.${util.fileTypes.page}`);
