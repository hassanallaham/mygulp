...
function getPageTemplate(data) {
	...
	if (typeof data.body === 'object') {
		return Object.keys(data.body)
			.map(
				(partial) =>
					`{{#with body.${partial}}}{{> ${partial}}}{{/with}}`
			)
			.join('\n');
	} else {
		return '';
...
function render(file, enc, cb, stream) {
	...
	try {
		...
		if (!layoutTemplate) {
			if (layout === 'default') {
				throw new Error(
					'Builder error: you must have a layout named "default".'
				);
			} else {
				throw new Error(
					'Builder error: no layout named "' + layout + '" exists.'
				);
			}
		}
		...
		this.Handlebars.registerHelper(
			'ifpage',
			require('../builtinHelpers/ifPage')(page.name)
		);
		this.Handlebars.registerHelper(
			'unlesspage',
			require('../builtinHelpers/unlessPage')(page.name)
		);
		...
		if (page.data.template) {
			...
			for (let pageItemData of pages) {
				...
				let { pagePath, pageUrl } = page.getPathFromUrl(
					pageItemData.url
				);
				...
				let pageData = Object.assign(
					...
					page.data,
					{
						...
						description:
							pageItemData.description || page.data.description,
						keywords: pageItemData.keywords || page.data.keywords,
						name: pageItemData.name || page.data.name,
						breadcrumb:
							pageItemData.breadcrumb || page.data.breadcrumb,
					},
					{
		...
		if (layoutTemplate) {
			...
			this.Handlebars.registerPartial(
				'body',
				'Builder: template could not be parsed <br> \n <pre>{{error}}</pre>'
			);
			...
			file.contents = new Buffer(
				'<!DOCTYPE html><html><head><title>Builder error</title></head><body><pre>' +
					e +
					'</pre></body></html>'
			);
		}
