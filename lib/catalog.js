...
const yaml = require('js-yaml'); // YAML 1.2 parser / writer for JavaScript
const sharp = require('sharp'); // to convert large images in common formats to smaller, web-friendly JPEG, PNG, WebP and AVIF images of varying dimensions.
const _ = require('lodash');
const util = require('./util');
const yargs = require('yargs');

const xml = require('xml-js'); // Convert XML text to Javascript object / JSON text (and vice versa).
const moment = require('moment'); // A JavaScript date library for parsing, validating, manipulating, and formatting dates.
	...
	const ymlCatalog = (categories, offers) => ({
		...
		yml_catalog: {
			...
			shop: {
				...
				categories: {
					category: Object.keys(categories).map(
						(categoryID, index) => ({
							_attributes: {
								id: categoryID,
							},
		...
		(async function catalog(options) {
			...
			try {
				...
				if (!source.isDirectory())
					throw Error('Source is not directory');
			} catch (err) {
			...
			try {
				...
				if (!target.isDirectory())
					throw Error('Target is not directory');
			} catch (err) {
						...
						if (accepted.indexOf(extType) != -1) {
							if (isTypedFiles) {
								for (let typeName of Object.keys(types)) {
									if (
										types[typeName].indexOf(extType) != -1
									) {
										if (!data[typeName])
											data[typeName] = [];
			...
			await fs.writeFile(
				path.join(config.source, '__tree.yml'),
				yaml.dump(files)
			);
			...
			const buildData = async function (
				...
				const isRoot = () =>
					Object.keys(inheritedSettings).length === 0;
				const isCategory = (t) =>
					t['folders'] != undefined &&
					Object.keys(t['folders']).length > 0;
				const generateString = (key, value, data) => {
					value = `${value}`;
					let toReplace = value
						.find(/\$\{([\w\.\_]+)\}/gi)
						.map((item) => ({ tag: item[0], path: item[1] }));
				...
				if (tree.data) {
					if (tree.data.indexOf('settings.yml') != -1) {
						customSettings = yaml.load(
							await fs.readFile(path.join(root, 'settings.yml'))
						);
					}
				}
				...
				let data = [customData];
				let tasks = [];
				let generatedData = settings.dataTemplate
					? generateData(settings.dataTemplate, customData, {
				...
				if (isCategory(tree)) {
					...
					for (let folderName in tree.folders) {
						...
						let { data: folderData, tasks: folderTasks } =
							await buildData(
				...
				if (tree.images)
					for (let imageItem of tree.images) {
						let extType = path
							.extname(imageItem)
							.slice(1)
							.toLowerCase();
						...
						let task = {
							...
							source: path.relative(
								settings.root,
								path.join(root, imageItem)
							),
							copy: path.relative(
								settings.root,
								path.join(dest, targetName)
							),
							preview: path.relative(
								settings.root,
								path.join(dest, previewName)
							),
						};
				...
				if (tree.videos)
					for (let videoItem of tree.videos) {
						let extType = path
							.extname(videoItem)
							.slice(1)
							.toLowerCase();
						...
						let task = {
							...
							source: path.relative(
								settings.root,
								path.join(root, videoItem)
							),
							copy: path.relative(
								settings.root,
								path.join(dest, videoItem)
							),
							preview: null,
						};
			...
			let { data, tasks } = await buildData(
				config.source,
				files,
				path.join(config.target, config.name)
			);
			await fs.writeFile(
				path.join(config.source, 'index.yml'),
				yaml.dump(data)
			);
			await fs.writeFile(
				path.join(config.source, '__tasks.yml'),
				yaml.dump(tasks)
			);
			await fs.writeFile(
				path.join(config.public, 'pricelist.xml'),
				xml.js2xml(ymlCatalog(productCategories, productOffers), {
					compact: true,
					spaces: 4,
				})
			);
			...
			for (let taskItem of tasks) {
				...
				let targetFolder = path.resolve(
					config.source,
					path.dirname(taskItem.copy)
				);
				let copyName = path.basename(taskItem.copy);
				let previewName = taskItem.preview
					? path.basename(taskItem.preview)
					: null;
				...
				if (copyName) {
					await fs.copy(
						sourcePath,
						path.join(targetFolder, copyName)
					);
				}
				if (taskItem.type == 'image' && previewName) {
					...
					await image
						.resize(taskItem.resize)
						.toFile(path.join(targetFolder, previewName));
				}
			}
