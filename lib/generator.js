...
module.exports = function generator(options) {
	...
	return function (done) {
		...
		exec('git status', (error, stdout, stderr) => {
			const isSaved =
				stdout.indexOf('nothing to commit, working tree clean') !== -1;
			util.log(
				`check = ${isCheck}; force = ${isForced}; restore = ${isRestored}; saved = ${isSaved}`
			);

			if (!isSaved && !isForced) {
				util.log(
					'You have unsaved chaged in working directory. Commit it or use --force param instead.'
				);
				return done();
			}
			...
			if (isRestored) {
				...
				let site = isRewrite
					? {}
					: yaml.safeLoad(fs.readFileSync(sitemap));
				let pageFiles = util.loadFiles(
					pages,
					`**/*.${util.fileTypes.page}`
				);
				let restoredPages = 0;
				...
				for (let pageFilePath of pageFiles) {
					...
					let pageName = pageFilePath
						...
						.split(path.sep)
						.join('_');
					let pageInstance = Page.fromFile(
						pages,
						pageFilePath,
						pageName
					);
			...
			let pageFiles = util
				.loadFiles(pages, `**/*.${util.fileTypes.page}`)
				.map((page) =>
					path
						.relative(pages, page)
						...
						if (_.isArray(site[pageName].body)) {
							...
							for (let widget of site[pageName].body) {
								...
								if (!isCheck) {
									...
									fs.ensureFileSync(
										path.join(partials, `${widget}.html`)
									);
								}
							}
						}
